...
export default async function cr60_pdf(ctx: Context, ouid: string, type: "pdf" | "json") {
  // BODY CHECK
  ctx.assert(/\d{10}/.test(ouid), 400, "please send valid student number. " + ouid);

  // DOWNLOAD IMAEG AS BUFFER
  // const s = new Date().getTime();
  const pdf_stream: Stream = await ctx.axios(PDF_GRADE_REPORT(ouid)).then(res => res.data);
  // const t = new Date().getTime();
  // console.log("stream load", (t - s) / 1000);
      ...
      if (typeof val == "string") {
        ...
        switch (val.slice(0, -5).trim()) {
          ...
          case "SUMMER SESSION":
            ...
            break;
          default:
            ctx.throw(500, `semester not exit in '${curSemester}'`);
        }
        ctx.assert(
